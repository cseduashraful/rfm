...
class RelBenchDuckDBPipeline:
    ...
    def _make_dataset(self):
        ...
        if self.config.cache_root is not None:
            kwargs["cache_dir"] = str(Path(self.config.cache_root) / self.config.dataset_name)

        cache_dir = Path(kwargs["cache_dir"])
        self.config.relbench_cache_dir = cache_dir
    ...
    def _ensure_dataset_cache_dir(self) -> None:
        if self.config.relbench_cache_dir is None:
            self._make_dataset()
        self.config.dataset_cache_dir.mkdir(parents=True, exist_ok=True)
